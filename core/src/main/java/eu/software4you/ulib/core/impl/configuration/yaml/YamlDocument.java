...
public class YamlDocument implements YamlConfiguration, Keyable<String> {
    ...
    public <T> Optional<T> get(@NotNull String path) {
        ...
        return resolveChild(path)
                ...
                .map(o -> {
                    ...
                    return Expect.<T, ClassCastException>compute(() -> (T) o).getValue();
                });
    }
    ...
    @Override
    @NotNull
    public Optional<YamlConfiguration> getSub(@NotNull String path) {
        return resolveChild(path)
                .map(Pair::getSecond)
    ...
    @Override
    public @NotNull Configuration subAndCreate(@NotNull String path) {
        return getSub(path)
                .orElseGet(() -> createSub(path));
    }

    @Override
    public @NotNull Collection<YamlConfiguration> getSubs() {
